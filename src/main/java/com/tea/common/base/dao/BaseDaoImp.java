package com.tea.common.base.dao;
...
public abstract class BaseDaoImp extends JdbcDaoSupport implements BaseDao {
	...
	static Log log = LogFactory.getLog(BaseDaoImp.class);

	public abstract void setDs(DataSource ds);

	protected JdbcTemplate createJdbcTemplate(DataSource dataSource) {
		return new JdbcTemplateTimeFix(dataSource);
	}
	...
	@Override
	public <T> List<T> query(String sql, Object[] args, Class<T> resultType) throws DataAccessException {
		SqlRowSetEx sqlRowSetEx = this.queryForRowSet(sql, args);
		return sqlRowSetEx.fillObject(resultType);
	}
	
	
	public static void main(String[] args) {
		String sql = "select * from (select * from x) y where 1=1 limit 0,10";
		sql = "select * from (select * from x) y where 1=1";
		
		StringBuilder countSql = new StringBuilder("select count(1) ");
//		countSql.append(sql.substring(sql.lastIndexOf("from")));
		countSql.append(sql.substring(sql.indexOf("from"), sql.lastIndexOf("limit")));
