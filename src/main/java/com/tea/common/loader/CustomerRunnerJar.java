...
public final class CustomerRunnerJar {
	...
	public static List<File> getRunAppJar()
	{
		...
		Properties p =	PropertiesUtil.loadResource(CustomerSetting.localWindowsConfigName, CustomerSetting.linuxConfigNameService);
	...
	{
		ArrayList<File> list = new ArrayList<>();
		Properties p =	PropertiesUtil.loadResource(CustomerSetting.localWindowsConfigName, CustomerSetting.linuxConfigNameWeb);
		String extDir = String.valueOf(p.get("ExtJar"));
		if(extDir != null && extDir.length() > 0)
		{
			File dir = new File(extDir);
